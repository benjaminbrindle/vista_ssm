...
class TuningEMlgssm(object):
    ...
    def _init_em_lgssm(self,time_points):
        """
        ...
        lgssm = EMlgssm(
            state_mat=np.eye(self.d_x) * 0.01,
        ...
        lgssm.A_list = np.array([random_qr_matrix(self.d_x) for t in range(len(lgssm.A_list))])
        lgssm.Gamma_list = np.array([np.eye(self.d_x) * 0.05 for t in range(len(lgssm.Gamma_list))])
        lgssm.Sigma_list = np.array([np.eye(self.d_y) * 0.05 for t in range(len(lgssm.Sigma_list))])
    ...
    def fit(self, 
        y, time_points, ux=None, uy=None, max_iter=10, epsilon=0.01, fix_param=[], n_lgssm=10, random_state=None):
        """
        Run EM algorithm.
        ...
        for j in range(n_lgssm):
            ...
            # Set parameters
            lgssm = self._init_em_lgssm(time_points)
            ...
            ll_set.append(lgssm.run_e_step(stage=False).compute_likelihoods(
