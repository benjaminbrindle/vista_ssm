...
class InitEMmlgssm(TuningEMlgssm):
    ...
    def _tuning(self, index):
        ...
        params_array = self.fit(
            y=self.set_y[index], time_points=self.set_time[index], ux=self.u_x, uy=self.u_y, 
    ...
    def _kmeans(self, params_vec):
        ...
        return centers_vec, weights, labels
    ...
    def fit_tuning(self, 
        ...
        # k-means clustering
        centers_vec, weights, labels = self._kmeans(params_vec)

        # vec to array
        initial_params = self._vec_to_array(centers_vec)

        initial_params['weight'] = weights 
        initial_params['labels_test']=labels
