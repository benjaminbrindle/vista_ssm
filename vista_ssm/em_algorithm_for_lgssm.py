...
class EMlgssm(KalmanFS):
    ...
    def _update_Gamma(self):
        ...
        cov = np.sum(
            (self.e_xtxt[1:] 
            - np.einsum("nij,nlj->nil", 
                  self.A_list, self.e_xtxt_1
            ) 
            - np.einsum("nlj,nij->nli", 
                  self.e_xtxt_1, self.A_list
            )
            + np.einsum("nij,njk,nlk->nil", 
                  self.A_list, self.e_xtxt[:-1], self.A_list
            ))/self.dt[1:,None,None]
            , axis=0
        )
        ...
        if stage==True:
            ...
            if np.isnan(self.y).any():
                ...
                for t in range(len(self.y)):
                    ...
                    YY[np.ix_(y_na,y_na)] += (self.C[np.ix_(y_na)]@self.e_xtxt[t]@self.C[np.ix_(y_na)].T + self.Sigma_list[t][np.ix_(y_na,y_na)])*self.dt[t]
    ...
    def fit(self, y, ux=None, uy=None, max_iter=10, epsilon=0.01, fix_param=[]):
        ...
        for i in range(max_iter):
            ...
            # Run E-step & M-step
            self.run_e_step(stage=True).run_m_step()
