...
class KalmanFS(object):
    ...
    def __init__(self, 
        ...
        diffs = self.T[1:]-self.T[:-1]
        self.dt = np.insert(diffs,0,diffs[0])
            
        self.A_list = np.array([self.A*t + np.eye(self.d_x) for t in self.dt])[1:]
        self.Gamma_list = np.array([self.Gamma*t for t in self.dt])[1:]
        self.Sigma_list = np.array([self.Sigma/t for t in self.dt])
    ...
    def _predict_y(self, x_mean, x_cov, t, uy_t=None):
        """
        Predict y[t].
        ...
        t: int
            Time index for fixed parameters
    ...
    def _update_x(self, x_mean, x_cov, y_t, t, uy_t=None):
        """
        Update x[t].
        ...
        return (gain_f, x_mean, (x_cov+x_cov.T)/2)


    def _predict_x(self, x_mean, x_cov, t, ux_t=None):
        """
        Predict x[t].
        
        Functions
        ---------
        x_mean <- A[t]*x_mean (+ B*u_x[t])
        x_cov <- A[t]*x_cov*A^T + Gamma[t]
        ...
        x_mean = np.dot(self.A_list[t], x_mean)
        x_cov = np.dot(self.A_list[t], np.dot(x_cov, self.A_list[t].T)) + self.Gamma_list[t]

        if add_input_to_state(self.B):
            x_mean += np.dot(self.B, ux_t)

        return (x_mean, (x_cov+x_cov.T)/2)
    

    def _online_filter(self, x_mean, x_cov, y_t, t, ux_t=None, uy_t=None):
        """
        ...
        (x_mean, x_cov) = self._predict_x(
            x_mean=x_mean, x_cov=x_cov, t=t, ux_t=ux_t
        )
        # Update 
        (_, x_mean, x_cov) = self._update_x(
            x_mean=x_mean, x_cov=x_cov, y_t=y_t, t=t, uy_t=uy_t
        )
        
        return (x_mean, (x_cov+x_cov.T)/2)

    
    def _smooth_x(self, x_mean_f, x_cov_f, x_mean_p, x_cov_p, x_mean_s, x_cov_s, t):
        """
        Smooth x[t].

        Functions
        ---------
        gain_s <- x_cov_f*A[t]^T*x_cov_p^{-1}
    ...
    def run_filter(self, y, ux=None, uy=None):
        ...
        for t in range(len_y-1):
            ...
            (x_means_p[t+1], x_covs_p[t+1]) = self._predict_x(
                x_mean=x_means_f[t], x_cov=x_covs_f[t], t=t, ux_t=self.u_x[t]
            )
    ...
    def run_smoother(self, x_means_f, x_covs_f, x_means_p, x_covs_p):
        ...
        for t in reversed(range(len_y-1)):
            ...
            (gains_s[t], x_means_s[t], x_covs_s[t]) = self._smooth_x(
                ...
                x_mean_s=x_means_s[t+1], x_cov_s=x_covs_s[t+1], t=t
            )
